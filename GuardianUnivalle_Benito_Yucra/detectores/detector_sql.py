...
import base64        # codificar/decodificar datos en Base64, útil para manejar claves o tokens.
import os            # generar bytes aleatorios (os.urandom).
import json          # serializar y deserializar datos en formato JSON.
import time          # timestamps, delays, mediciones, etc.
...
import urllib.parse  # parsear URLs, querystrings, codificar/decodificar parámetros.
import ipaddress     # validar y manipular direcciones IP (IPv4 e IPv6).
...
AEAD_CHOICE = getattr(settings, "SQLI_DEFENSE_AEAD", "AESGCM").upper()  
...
HASH_CHOICE = getattr(settings, "SQLI_DEFENSE_HASH", "SHA256").upper()
...
def is_fingerprint_blocked(fingerprint: str) -> bool:
    if not fingerprint:
        return False
    return bool(cache.get(f"sqli_block_fingerprint:{fingerprint}"))  
...
def cache_block_ip_with_backoff(ip: str, fingerprint: str = ""):
    ...
    if fingerprint:  
        cache.set(f"sqli_block_fingerprint:{fingerprint}", True, timeout=timeout)  
...
class SQLIDefenseCryptoMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        if is_fingerprint_blocked(fingerprint):
            ...
            logger.warning(f"[SQLiBlock:Fingerprint] Fingerprint={fingerprint} IP={client_ip} - I   ntento persistente de acceso bloqueado.")
