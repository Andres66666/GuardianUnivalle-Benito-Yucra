...
CSRF_DEFENSE_EXCLUDED_API_PREFIXES = getattr(settings, "CSRF_DEFENSE_EXCLUDED_API_PREFIXES", [])

CSRF_DEFENSE_BLOCK = bool(getattr(settings, "CSRF_DEFENSE_BLOCK", True))
CSRF_DEFENSE_BLOCK_SCORE = float(getattr(settings, "CSRF_DEFENSE_BLOCK_SCORE", 0.35))  
CSRF_DEFENSE_BLOCK_REQUIRE_ORIGIN_MISMATCH = bool(
    getattr(settings, "CSRF_DEFENSE_BLOCK_REQUIRE_ORIGIN_MISMATCH", True)
)
...
AEAD_CHOICE = getattr(settings, "CSRF_DEFENSE_AEAD", "AESGCM").upper()  
...
def should_analyze_csrf_request(request, params: List[str]) -> bool:
    ...
    if method == "GET":
        ...
        if origin or referer:
            return True
        for p in params:
            if (p or "").lower() in SENSITIVE_PARAMS:
                return True
    return False

def is_sensitive_field(name: str) -> bool:
    return (name or "").lower() in set(SENSITIVE_FIELDS)

def encrypt_field_value(value: str, context: bytes) -> Dict[str, str]:
...
class CSRFDefenseMiddleware(MiddlewareMixin):
    def process_request(self, request):
        excluded_prefixes = getattr(settings, "CSRF_DEFENSE_EXCLUDED_API_PREFIXES", [])
        ...
        if not should_analyze_csrf_request(request, params):
            return None
        ...
        if "application/json" in content_type:
            ...
            if origin and host_from_header(origin) != host:
                descripcion.append("JSON POST/PUT desde origen externo (posible CSRF)")
        ...
        if not descripcion or total_signals < CSRF_DEFENSE_MIN_SIGNALS:
            return None
        ...
        origin = request.META.get("HTTP_ORIGIN", "") or ""
        referer = request.META.get("HTTP_REFERER", "") or ""
        origin_or_referer_present = bool(origin or referer)

        origin_mismatch = (not origin_matches_host(request)) if origin_or_referer_present else False
        missing_token = (not has_csrf_token(request))
        strong_csrf = missing_token and origin_mismatch
        score_csrf = s_csrf >= CSRF_DEFENSE_BLOCK_SCORE

        if CSRF_DEFENSE_BLOCK_REQUIRE_ORIGIN_MISMATCH:
            should_block = CSRF_DEFENSE_BLOCK and (strong_csrf or (score_csrf and origin_mismatch))
        else:
            should_block = CSRF_DEFENSE_BLOCK and (strong_csrf or score_csrf)
        ...
        request.csrf_attack_info = {
            ...
            "payload": (
                ...
                if payload_summary
                else json.dumps({"full_payload": full_payload[:500]}, ensure_ascii=False)
            ),
            "score": s_csrf,
            "url": url,
        ...
        logger.warning(
            "CSRF detectado desde IP %s: %s ; path=%s ; Content-Type=%s ; score=%.2f ; url=%s ; blocked=%s ; payload_summary=%s",
            client_ip,
            descripcion,
            request.path,
            content_type,
        ...
        if should_block:
            ...
            request.csrf_block_response = HttpResponseForbidden(
                "Acceso denegado por CSRF sospechoso."
            )
            return None
